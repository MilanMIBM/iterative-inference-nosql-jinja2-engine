...
def _():
    ...
    data_validator_library_endpoint = os.getenv("DATA_VALIDATOR_LIBRARY_ENDPOINT", "")
...
def _(organizational_context):
    ...
    output_language = (
        _org_docs[0].get("language") if _org_docs and len(_org_docs) > 0 else "English"
    )
...
def _(all_validated_results, validation_enabled):
    ...
    if validation_enabled and all_validated_results:
        ...
        for _res in all_validated_results:
            ...
            _kind = "Validated" if _validated is not None else "Parsed (unvalidated)"
            _blocks.append(
                mo.vstack(
                    [
