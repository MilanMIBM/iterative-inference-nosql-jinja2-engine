...
def build_additional_query_params(
    search_terms: Optional[List[str]] = None,
    search_terms_match: str = "any",
    field: str = "FT",
) -> Optional[str]:
    """
    Build a single TED expert-query clause from a list of free-text search
    terms, suitable for AND-ing into an existing query.

    Args:
        search_terms (List[str], optional): Free-text terms to search for,
            e.g. ["time-series database", "timeseries"]. Falsy entries are
            dropped.
        search_terms_match (str): "any" joins the terms with OR (match any
            term); "all" joins with AND (match every term). Defaults to "any".
        field (str): The text field to search on. Defaults to "FT".

    Returns:
        Optional[str]: A query clause such as
        '(FT~"time-series database" OR FT~"timeseries")', a bare clause for a
        single term, or None if no usable terms were given.
    """
    terms = [t for t in (search_terms or []) if t]
    if not terms:
        return None
    joiner = " OR " if search_terms_match == "any" else " AND "
    clause = joiner.join(f'{field}~"{t}"' for t in terms)
    return f"({clause})" if len(terms) > 1 else clause
...
def search_ted_notices(
    ...
    Args:
        ...
        search_terms (List[str], optional): Free-text terms to search for,
            e.g. ["time-series database", "timeseries"]. Wrapped as
            ``field~"term"`` clauses and AND-ed into the query.
        search_terms_match (str): "any" joins the terms with OR (match any
            term); "all" joins with AND (match every term). Defaults to "any".
        field (str): The text field the search terms are matched against.
            Defaults to "FT".
    ...
    # Add optional free-text search terms (e.g. "time-series database").
    terms_clause = build_additional_query_params(
        search_terms, search_terms_match, field
    )
    if terms_clause:
        query_parts.append(terms_clause)
...
def extract_downloaded_files_batch(
    ...
    * ``batch_extract_bytes_sync`` does not raise on a single bad file; it
    ...
    with mo.status.progress_bar(
        ...
        try:
            _results = batch_extract_bytes_sync(_data_list, _mime_types, _active_config)
